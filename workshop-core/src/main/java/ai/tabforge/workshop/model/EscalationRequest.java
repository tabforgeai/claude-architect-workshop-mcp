...
public record EscalationRequest(
     ...
     * Is invited by EscalationHandler.
     * <p>
     * So, when the OrchestratorAgent starts agent execution, then, if agent finds critical finding during file procession, and before continuing with the next file, it will notify the orchestrator of the escalation, and stop the work.  Let's say for now, before stopping, that the agent will call the **escalate()** method of the OrchestratorAgent (not implemented yet, will be latter).  By calling this method, the agent sends the reviewId, as well as information about his [finding](https://tabforgeai.github.io/claude-architect-workshop-mcp/ai/tabforge/workshop/model/Finding.html) for the file it processes ( Severity - CRITICAL, INFO, WARNING, then  lineNumber, ...)
      Knowing rewiewId and Finding, OrchestratorAgent.escalate() then sends the Finding on deciding what to do - let's call the mechanism that decides what to do next - the **Escalation Handler** the only job of that mechanism (we don't have it yet) will be to:
      
      <ol>
        <li>check if confidence ≥ 0.70</li>
        <li>create an EscalationRequest</li>
        <li>return just created EscalationRequest to the OrchestratorAgent</li>
      </ol>
      </p>
      
      <p>
      So, the 'question' attribute of this record will be generated by calling this method by the EscalationHandler - factory method. 
      </p>
